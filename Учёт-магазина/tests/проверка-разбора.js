...
console.log('\n— Сколько выгрузок 1С держим в памяти');
{
  /* Выгрузки копятся по месяцам и живут только в памяти: десять лет работы
     магазина на 3000 наименований — это 77 МБ, которые перечитываются при
     каждом запуске. Старые периоды отбрасываем, но ручной учёт не трогаем
     никогда — он в базе, а не в файлах. */
  check('24 месяца назад от сентября 2026 — это октябрь 2024',
    WM.monthStart('2026-09-17', 23) === '2024-10-01', WM.monthStart('2026-09-17', 23), '2024-10-01');
  check('через границу года считает верно',
    WM.monthStart('2026-02-10', 3) === '2025-11-01', WM.monthStart('2026-02-10', 3), '2025-11-01');

  const строки = [
    { to: '2024-09-30', name: 'позапрошлый год' },
    { to: '2024-10-01', name: 'ровно на границе' },
    { to: '2026-09-30', name: 'этот месяц' },
    { name: 'период неизвестен' }
  ];
  const о = WM.pruneOldPeriods(строки, 24, '2026-09-17').map(function (r) { return r.name; });
  check('старое выброшено', о.indexOf('позапрошлый год') < 0, о.join(', '), 'без позапрошлого года');
  check('граница включительно — месяц не теряется', о.indexOf('ровно на границе') >= 0, о.join(', '), 'граница на месте');
  check('свежее осталось', о.indexOf('этот месяц') >= 0, о.join(', '), 'на месте');
  check('строку без периода не выбрасываем: судить о возрасте не по чему',
    о.indexOf('период неизвестен') >= 0, о.join(', '), 'на месте');

  check('0 месяцев — не ограничивать',
    WM.pruneOldPeriods(строки, 0, '2026-09-17').length === 4,
    WM.pruneOldPeriods(строки, 0, '2026-09-17').length, 4);
  check('пустой настройке тоже верим как «не ограничивать»',
    WM.pruneOldPeriods(строки, '', '2026-09-17').length === 4,
    WM.pruneOldPeriods(строки, '', '2026-09-17').length, 4);
  check('1 месяц оставляет только текущий',
    WM.pruneOldPeriods(строки, 1, '2026-09-17').length === 2,
    WM.pruneOldPeriods(строки, 1, '2026-09-17').length, 2);

  /* Главное: ручной учёт сюда не попадает ни при каких настройках.
     Смены и расходы лежат в базе, у них нет поля периода выгрузки. */
  const смены = [{ type: 'Смена', date: '2019-01-01', zCash: 40000, zCashless: 10000 },
                 { type: 'Расход', date: '2018-05-05', amount: 15000, category: 'Аренда' }];
  const итог = WM.totals(смены, {});
  check('СМЕНА 2019 ГОДА СЧИТАЕТСЯ ПОЛНОСТЬЮ, СКОЛЬКО БЫ ЛЕТ НИ ПРОШЛО',
    итог.revenue === 50000, итог.revenue, 50000);
  // и обрезка выгрузок на неё не влияет: у записей учёта нет поля периода
...
console.log('\n— Кассовая книга за один месяц');
{
  /* Смотрим один месяц — начинать надо с того, что перешло с прошлого.
     ...
     открывалась суммой, которая лежала в сейфе в апреле. Увидел глазами на
     полугоде данных: книга говорила 5 миллионов там, где в сейфе 34. */
  const счета = [{ id: 'с', name: 'Сейф', kind: 'cash', opening: 10000, defaultCash: true },
                 { id: 'б', name: 'Счёт', kind: 'bank', opening: 0 }];
  const r = [
    { type: 'Смена', date: '2026-08-15', zCash: 50000, payouts: 0, openCash: 0,
      kept: 0, received: 50000, toAccount: 'с', cashlessAccount: 'б' },
    { type: 'Смена', date: '2026-09-10', zCash: 30000, payouts: 0, openCash: 0,
      kept: 0, received: 30000, toAccount: 'с', cashlessAccount: 'б' }
  ];
  const сент = WM.cashBook(r, {}, '2026-09-01', '2026-09-30', счета);
  check('КНИГА ЗА МЕСЯЦ НАЧИНАЕТСЯ С ТОГО, ЧТО ПЕРЕШЛО С ПРОШЛОГО',
    сент.open === 60000, сент.open, 60000);
  check('и заканчивается остатком сейфа на конец месяца',
    сент.close === WM.safeOnHand(r, {}, null, счета), сент.close,
    WM.safeOnHand(r, {}, null, счета));

  // Первый месяц: переходить нечему, начинаем с начального остатка счёта
  const авг = WM.cashBook(r, {}, '2026-08-01', '2026-08-31', счета);
  check('в первом месяце начало — это начальный остаток счёта',
    авг.open === 10000, авг.open, 10000);

  // Без периода книга берёт весь срок и сходится с сейфом
  const весь = WM.cashBook(r, {}, null, null, счета);
  check('без периода книга сходится с наличными',
    весь.close === WM.safeOnHand(r, {}, null, счета), весь.close,
    WM.safeOnHand(r, {}, null, счета));
}

console.log('\n— Смена объясняет свои выплаты сама');
{
  /* Владелец вписывает выплаты кассира строками «кому и за что» прямо в
     форме смены. Требовать, чтобы он расписал их второй раз отдельными
     расходами, значит заставлять делать одну работу дважды — а потом ещё
     и ругаться на него, что не сходится. */
  const новая = [{ type: 'Смена', date: '2026-09-14', zCash: 74841, payouts: 10760,
    openCash: 5000, kept: 5000, received: 57969,
    payoutList: [{ name: 'Молокозавод', sum: 8000 }, { name: 'Вода', sum: 2760 }] }];
  const c = WM.tillPayoutCheck(новая, null, {});
  check('СТРОКИ «КОМУ И ЗА ЧТО» И ЕСТЬ РАСШИФРОВКА', c.left === 0,
    'осталось ' + c.left, 0);
  check('и каждая строка названа своим именем',
    c.parts['Молокозавод'] === 8000 && c.parts['Вода'] === 2760,
    Object.keys(c.parts).join(', '), 'Молокозавод и Вода');

  // Старая смена расшифровки в себе не несёт — её по-прежнему ждём со стороны
  const старая = [{ type: 'Смена', date: '2026-09-14', zCash: 74841, payouts: 10760,
    openCash: 5000, factCash: 62969 }];
  check('СТАРАЯ СМЕНА ПО-ПРЕЖНЕМУ ПРОСИТ РАСШИФРОВКУ',
    WM.tillPayoutCheck(старая, null, {}).left === 10760,
    WM.tillPayoutCheck(старая, null, {}).left, 10760);

  /* Расписали не всё — программа обязана это заметить. Иначе строки можно
     было бы вписать наполовину и не узнать об этом. */
  const неполная = [{ type: 'Смена', date: '2026-09-14', zCash: 74841, payouts: 10760,
    openCash: 5000, kept: 5000, received: 57969,
    payoutList: [{ name: 'Молокозавод', sum: 8000 }] }];
  check('РАСПИСАЛИ НЕ ВСЁ — ПРОГРАММА ЗАМЕЧАЕТ',
    WM.tillPayoutCheck(неполная, null, {}).left === 2760,
    WM.tillPayoutCheck(неполная, null, {}).left, 2760);
}
...
/* Владелец: «Начать просто, углубляться по желанию». Разбивка по
   поставщикам — это и есть «углубиться». Считать её надо честно: общий долг
   магазина по поставщикам НЕ раскладывается (в ручной цифре нет имён), а
   вот выплаты и привоз — раскладываются, и до копейки. */
/* Настройка «Долг поставщику веду» была заведена, но ни одна строка
   программы её не читала. Теперь читает — и старое значение пришлось бы
   обещанием, которого магазин не давал. */
console.log('\n— Переезд: настройка долга приведена к тому, что программа делает');
{
  const старая = { version: 3, dds: [], accounts: [],
    settings: { debtMode: 'по накладным', storeName: 'Мой магазин' } };
  const н = STORE.поднять(JSON.parse(JSON.stringify(старая)));
  check('база поднялась до нынешней версии', н.version === STORE.ВЕРСИЯ, н.version, STORE.ВЕРСИЯ);
  check('ОБЕЩАНИЕ, КОТОРОГО НЕ БЫЛО, УБРАНО',
    н.settings.debtMode === 'общей суммой', н.settings.debtMode, 'общей суммой');
  check('и остальные настройки магазина не тронуты',
    н.settings.storeName === 'Мой магазин', н.settings.storeName, 'Мой магазин');
  const дважды = STORE.поднять(JSON.parse(JSON.stringify(н)));
  check('повторный переезд ничего не портит',
    дважды.settings.debtMode === 'общей суммой', дважды.settings.debtMode, 'общей суммой');
  check('база без настроек переезд переживает',
    STORE.поднять({ version: 3 }).version === STORE.ВЕРСИЯ, 'пережила', 'пережила');
}
...
console.log('\n— Разбивка по поставщикам');
{
  ...
  const plans = [
    { due: '2026-09-01', supplier: 'Рамми', amount: 10000, status: 'Запланирована' },
    { due: '2026-08-20', supplier: 'Рамми', amount: 2500.55, status: 'Запланирована' },
    { due: '2026-09-05', supplier: 'Ока', amount: 5000, status: 'Запланирована' },
    { due: '2026-09-20', supplier: 'Ока', amount: 7000, status: 'Запланирована' },
    { due: '2026-09-02', supplier: 'Ока', amount: 3000, status: 'Оплачена' },
    { due: '2026-09-02', supplier: 'Ока', amount: 9999, status: 'Отменена' }
  ];
  const supplies = [
    { supplier: 'Ока', sum: 120000, paid: 100000, docs: 12 },
    { supplier: 'Молокозавод', sum: 40000, paid: 40000, docs: 8 }
  ];
  const b = WM.supplierBreakdown(plans, supplies, t);
  const по = {};
  b.forEach(function (r) { по[r.supplier] = r; });

  check('каждый поставщик одной строкой, а не по разу на накладную',
    b.length === 3, b.length, 3);
  check('СНАЧАЛА ТОТ, У КОГО ПРОСРОЧКА', b[0].supplier === 'Рамми', b[0].supplier, 'Рамми');
  check('просроченное сложилось до копейки',
    по['Рамми'].overdue === 12500.55, по['Рамми'].overdue, 12500.55);
  check('оплаченная выплата в долг не идёт', по['Ока'].planned === 12000,
    по['Ока'].planned, 12000);
  check('ОТМЕНЁННАЯ ВЫПЛАТА НЕ СЧИТАЕТСЯ НИГДЕ',
    по['Ока'].planned === 12000 && по['Ока'].paidPlans === 3000,
    по['Ока'].planned + ' / ' + по['Ока'].paidPlans, '12000 / 3000');
  check('«сегодня» и «скоро» — отдельным числом', по['Ока'].soon === 5000,
    по['Ока'].soon, 5000);
  check('привоз из 1С лёг к своему поставщику',
    по['Ока'].brought === 120000 && по['Ока'].docs === 12,
    по['Ока'].brought + ' / ' + по['Ока'].docs, '120000 / 12');
  check('поставщик только из 1С тоже в списке',
    !!по['Молокозавод'] && по['Молокозавод'].planned === 0,
    по['Молокозавод'] ? по['Молокозавод'].brought : 'нет', 40000);

  /* СУММА СТРОК ОБЯЗАНА СХОДИТЬСЯ С ОБЩИМИ ИТОГАМИ — иначе подробный
     взгляд противоречил бы простому, и владелец не знал бы, какому верить. */
  const pt = WM.planTotals(plans, t);
  const сумма = b.reduce(function (a, r) { return a + r.planned; }, 0);
  check('ПОДРОБНО И ПРОСТО ДАЮТ ОДНО И ТО ЖЕ',
    WM.safeRound(сумма) === pt.planned, WM.safeRound(сумма), pt.planned);
  const прос = b.reduce(function (a, r) { return a + r.overdue; }, 0);
  check('и просрочка тоже сходится', WM.safeRound(прос) === pt.overdue,
    WM.safeRound(прос), pt.overdue);

  /* Имя поставщика владелец пишет как придётся: «Ока» и «ока » — один и тот
     же человек, и в двух строках он стоять не должен. */
  const два = WM.supplierBreakdown([
    { due: '2026-09-01', supplier: 'Ока', amount: 1000, status: 'Запланирована' },
    { due: '2026-09-01', supplier: ' ока ', amount: 2000, status: 'Запланирована' }
  ], [], t);
  check('ОДИН ПОСТАВЩИК, НАПИСАННЫЙ ПО-РАЗНОМУ, — ОДНА СТРОКА',
    два.length === 1 && два[0].planned === 3000,
    два.length + ' строк, ' + (два[0] ? два[0].planned : 0), '1 строка, 3000');

  check('без данных — пустой список, а не поломка',
    WM.supplierBreakdown(null, null, t).length === 0, 0, 0);
}

/* Владелец: «не ограничивай программу в памяти, функциях, возможностях».
   Раньше выгрузки старше двух лет программа молча выбрасывала — владелец
   загружал десять лет, а сезонность за восемь из них не видел и не понимал
   почему. Теперь по умолчанию не выбрасывается ничего. */
console.log('\n— Программа ничего не выбрасывает молча');
{
  check('ПО УМОЛЧАНИЮ ПАМЯТЬ НЕ ОГРАНИЧЕНА',
    STORE.DEFAULT_SETTINGS.anaKeepMonths === 0,
    STORE.DEFAULT_SETTINGS.anaKeepMonths, 0);

  /* Десять лет выгрузок, по строке на месяц, кончая сегодняшним месяцем.
     Будущими месяцами не набиваем нарочно: обрезка их не трогает (и не
     должна — выгрузка с завтрашней датой это дело владельца), и тогда
     числа ниже читались бы не про то. */
  const десять = [];
  for (let г = 2017; г <= 2026; г++) {
    for (let м = 1; м <= 12; м++) {
      ...
      десять.push({ name: 'Хлеб', key: 'хлеб', revenue: 1000,
        from: г + '-' + мм + '-01', to: г + '-' + мм + '-' + дд });
    }
  }
  check('в примере ровно десять лет по месяцам', десять.length === 117, десять.length, 117);
  check('ДЕСЯТЬ ЛЕТ ОСТАЮТСЯ ДЕСЯТЬЮ ГОДАМИ',
    WM.pruneOldPeriods(десять, STORE.DEFAULT_SETTINGS.anaKeepMonths, '2026-09-18').length === 117,
    WM.pruneOldPeriods(десять, 0, '2026-09-18').length, 117);
  check('и пустая настройка тоже ничего не режет',
    WM.pruneOldPeriods(десять, '', '2026-09-18').length === 117 &&
    WM.pruneOldPeriods(десять, undefined, '2026-09-18').length === 117,
    'не режет', 'не режет');

  /* Но возможность ограничить остаётся: на слабом компьютере владелец может
     сказать «держи два года», и тогда программа обязана держать ровно два. */
  const два = WM.pruneOldPeriods(десять, 24, '2026-09-18');
  check('кто сам попросил ограничить — получит ровно столько', два.length === 24,
    два.length, 24);
  check('и обрежет самые старые, а не случайные',
    два[0].from === '2024-10-01' && два[два.length - 1].from === '2026-09-01',
    два[0].from + '…' + два[два.length - 1].from, '2024-10-01…2026-09-01');

  /* ГЛАВНОЕ. Обрезка касается только выгрузок 1С. Смены, расходы и табель —
     это работа владельца, её не выбрасывают ни при каких настройках. */
  const учёт = [
    { type: 'Смена', date: '2017-03-01', zCash: 50000 },
    { type: 'Расход', date: '2018-05-05', amount: 1000 },
    { type: 'День', date: '2019-01-01', debtTaken: 500 }
  ];
  check('РАБОТУ ВЛАДЕЛЬЦА НЕ ТРОГАЕТ НИКАКАЯ НАСТРОЙКА ПАМЯТИ',
    WM.pruneOldPeriods(учёт, 1, '2026-09-18').length === 3,
    WM.pruneOldPeriods(учёт, 1, '2026-09-18').length, 3);
}

/* Экран «Сезонность» показывал нули и прочерки в каждой строке при полном
   годе данных: он читал поля, которых движок не отдавал. Ошибка тихая —
   таблица рисуется, просто во всех клетках ноль. */
console.log('\n— Сезонность: в каком месяце магазин живёт');
{
  const GOODS = require(path.join(__dirname, '..', 'js', 'goods.js'));
  /* Магазин работает третий год. Январь случился трижды, июль — дважды.
     СУММА января при этом больше, чем мог бы дать один хороший январь. */
  const строки = [
    { date: '2024-01-15', amount: 100 },
    { date: '2025-01-10', amount: 100 },
    { date: '2026-01-10', amount: 100 },
    { date: '2025-07-10', amount: 300 },
    { date: '2026-07-10', amount: 300 }
  ];
  const s = GOODS.seasons(строки, null);
  const янв = s.months[0], июл = s.months[6], мар = s.months[2];

  check('месяц знает, в скольких годах он встретился',
    янв.years === 3 && июл.years === 2, янв.years + ' и ' + июл.years, '3 и 2');
  check('СРАВНИВАЮТСЯ СРЕДНИЕ ЗА ГОД, А НЕ СУММЫ',
    янв.avg === 100 && июл.avg === 300, янв.avg + ' и ' + июл.avg, '100 и 300');
  /* Главное правило этого экрана: январь не становится сезонным только
     потому, что случился чаще. По сумме январь (300) равен июлю (600)/2 —
     и по сумме они выглядели бы одинаково. */
  check('И ЧАСТЫЙ МЕСЯЦ НЕ СТАНОВИТСЯ СЕЗОННЫМ ПРОСТО ПОТОМУ, ЧТО ЧАЩЕ',
    янв.mark !== 'сезон' && июл.mark === 'сезон',
    'январь «' + янв.mark + '», июль «' + июл.mark + '»', 'сезон только июль');
  check('лучший и самый тихий месяц названы',
    s.best && s.best.name === 'июль' && s.worst && s.worst.name === 'январь',
    (s.best ? s.best.name : '—') + ' / ' + (s.worst ? s.worst.name : '—'), 'июль / январь');
  check('месяц без данных ничем не помечен',
    мар.years === 0 && мар.vs === null && мар.mark === '',
    мар.years + ' лет, метка «' + мар.mark + '»', '0 лет, без метки');
  /* Месяцы, которых не было, в «обычное» не идут: иначе магазин, открытый
     три месяца назад, выглядел бы так, будто девять месяцев в провале. */
  check('ПУСТЫЕ МЕСЯЦЫ НЕ ЗАНИЖАЮТ «ОБЫЧНЫЙ МЕСЯЦ»', s.avg === 200, s.avg, 200);
  check('без записей не ломается',
    GOODS.seasons([], null).monthsWithData === 0, 0, 0);
}

/* Экран «Заказы» читают стоя у полки: по нему решают, ехать за товаром
   сегодня или можно завтра. Он спрашивал у расчёта daysLeft и orderSum —
   а расчёт их не отдавал. В столбце «Хватит на» всегда стоял прочерк, «На
   сумму» показывало 0 ₽, и два фильтра срочности из трёх не находили
   ничего. С настоящей выгрузкой 1С — тоже. */
console.log('\n— Заказ товара: на сколько хватит и на сколько денег');
{
  // продали 60 штук за 30 дней = 2 в день; на полке 3 штуки
  const продажи = [{ name: 'Молоко', key: 'молоко', qty: 60 }];
  const остатки = [{ name: 'Молоко', key: 'молоко', qty: 3, buyPrice: 50, group: 'Молочка' }];
  const лучшие = { 'молоко': { price: 40, supplier: 'Молокозавод' } };
  const л = WM.ropList(продажи, остатки, 30, { leadDays: 2, safetyPct: 30 }, лучшие)[0];

  check('товар ниже точки заказа попал в список', !!л, л ? л.name : 'списка нет', 'Молоко');
  check('расход в день посчитан', л.demand === 2, л.demand, 2);
  check('НА СКОЛЬКО ДНЕЙ ХВАТИТ ОСТАТКА', л.daysLeft === 1.5, л.daysLeft, 1.5);
  check('и сколько заказать', л.order === 7, л.order, 7);
  /* Сумму считаем по ЛУЧШЕЙ цене поставщика (40), а не по той, по которой
     товар когда-то купили (50): заказывать будем по нынешней. */
  check('СУММА ЗАКАЗА — ПО ЛУЧШЕЙ ЦЕНЕ ПОСТАВЩИКА, А НЕ ПО СТАРОЙ ЗАКУПКЕ',
    л.orderSum === 280 && л.sum === 280, л.orderSum + ' / ' + л.sum, '280 / 280');
  check('и сказано, у кого дешевле', л.supplier === 'Молокозавод', л.supplier, 'Молокозавод');

  // товара нет вовсе — «хватит на 0 дней», а не «неизвестно»
  const пусто = WM.ropList(продажи, [{ name: 'Молоко', key: 'молоко', qty: 0, buyPrice: 50 }],
    30, { leadDays: 2, safetyPct: 30 }, лучшие)[0];
  check('кончившийся товар помечен как срочный',
    пусто.critical === true && пусто.daysLeft === 0,
    пусто.critical + ', на ' + пусто.daysLeft + ' дн.', 'true, на 0 дн.');

  // без цен поставщиков считаем по закупочной из остатков — но считаем
  const безЦен = WM.ropList(продажи, остатки, 30, { leadDays: 2, safetyPct: 30 }, null)[0];
  check('без цен поставщиков сумма всё равно считается, по закупочной',
    безЦен.orderSum === 350, безЦен.orderSum, 350);
}

/* ПОИСК, КОТОРЫЙ ОТВЕЧАЕТ. Владелец спрашивает словами: «сколько я должен
   Молокозаводу», «расходы за июль», «недостачи Ани». Раньше строка поиска
   вела в никуда — экрана с таким именем в программе не было вовсе.

   Здесь проверяется разбор вопроса: срок, имя и о чём вообще спросили.
   Считает ответ не этот модуль, а те же функции, которыми считают экраны, —
   поэтому проверять надо именно разбор. */
console.log('\n— Поиск понимает вопросы');
{
  const ASK = require(path.join(__dirname, '..', 'js', 'ask.js'));
  const QK = require(path.join(__dirname, '..', 'js', 'quick.js'));
  const t = WM.today(), год = +t.slice(0, 4);

  const S = { settings: { openDebtStart: 300000, diffCrit: 1000 }, state: {
    accounts: [{ id: 's', name: 'Сейф', kind: 'cash', opening: 50000, defaultCash: true },
      { id: 'b', name: 'Счёт', kind: 'bank', opening: 10000, defaultCashless: true }],
    dds: [
      { type: 'Смена', date: год + '-07-10', till: 'К1', shift: 'День', cashier: 'Аня',
        openCash: 0, zCash: 100000, zCashless: 20000, received: 95000, kept: 0 },
      { type: 'Смена', date: год + '-08-10', till: 'К1', shift: 'День', cashier: 'Антон',
        openCash: 0, zCash: 80000, zCashless: 0, received: 80000, kept: 0 },
      { type: 'Расход', date: год + '-07-15', category: 'Аренда', method: 'Наличные', amount: 40000 },
      { type: 'Расход', date: год + '-08-05', category: 'Аренда', method: 'Наличные', amount: 40000 }
    ],
    plans: [
      { due: год + '-09-20', supplier: 'Молокозавод', amount: 33000, status: 'Запланирована' },
      { due: год + '-09-25', supplier: 'Молокозавод', amount: 17000, status: 'Запланирована' },
      { due: год + '-09-01', supplier: 'Молокозавод', amount: 99000, status: 'Оплачена' }
    ],
    staff: [{ name: 'Аня' }, { name: 'Антон' }], timesheet: [], payouts: [] } };

  const спр = (в) => ASK.спросить(в, S, WM, QK);

  const долг = спр('Сколько я должен Молокозаводу');
  check('ВОПРОС ПРО ДОЛГ ПОСТАВЩИКУ ПОНЯТ', долг.есть && /Молокозавод/.test(долг.заголовок),
    долг.есть ? долг.заголовок : 'не понял', 'про Молокозавод');
  /* Оплаченная выплата в ответ не идёт: спрашивают, сколько ОСТАЛОСЬ. */
  check('и считает только неоплаченные выплаты', долг.число === 50000, долг.число, 50000);

  const июль = спр('расходы за июль');
  check('МЕСЯЦ В ВОПРОСЕ ПОНЯТ', /июль/.test(июль.подпись), июль.подпись, 'за июль');
  check('и расходы взяты только за него', июль.число === 40000, июль.число, 40000);

  const авг = спр('выручка за август');
  check('выручка за месяц', авг.число === 80000, авг.число, 80000);

  /* Имя в падеже. «Ани» и «Аня» — один человек, а «Антон» — другой: именно
     здесь легко перепутать, отрезав слишком много от конца слова. */
  const аня = спр('недостачи Ани');
  check('ИМЯ В ПАДЕЖЕ УЗНАЁТСЯ', /Аня/.test(аня.заголовок), аня.заголовок, 'про Аню');
  check('И АНЯ НЕ ПРЕВРАЩАЕТСЯ В АНТОНА', !/Антон/.test(аня.заголовок), аня.заголовок, 'без Антона');
  check('а Антон находится сам по себе', /Антон/.test(спр('недостачи Антона').заголовок),
    спр('недостачи Антона').заголовок, 'про Антона');

  const сейф = спр('сколько в сейфе');
  check('вопрос про деньги сейчас', сейф.есть && /сейф/i.test(сейф.заголовок),
    сейф.есть ? сейф.заголовок : 'не понял', 'про сейф');

  /* САМОЕ ВАЖНОЕ. Не понял — так и скажи. Молча показать не то хуже, чем
     честно сказать «не понял»: неверному ответу поверят. */
  check('НЕПОНЯТНЫЙ ВОПРОС ОСТАЁТСЯ НЕПОНЯТЫМ, А НЕ ПОЛУЧАЕТ СЛУЧАЙНЫЙ ОТВЕТ',
    !спр('погода в Грозном').есть && !спр('ъъъ').есть,
    'не понял оба', 'не понял оба');
  check('и пустой вопрос не выдумывает ответа',
    !спр('').есть && !спр('  ').есть, 'не понял', 'не понял');

  /* Про \b и кириллицу: в JavaScript граница слова считается по латинице,
     и «\bиюл» не находило слово «июль». Разбор месяца молча не работал, а
     ответ приходил «за всё время» — то есть за другой период. */
  check('МЕСЯЦ НАХОДИТСЯ В НАЧАЛЕ ВОПРОСА, В СЕРЕДИНЕ И В КОНЦЕ',
    /июль/.test(спр('июль расходы').подпись) &&
    /июль/.test(спр('расходы за июль').подпись) &&
    /июль/.test(спр('сколько потратил в июле').подпись),
    'везде', 'везде');
}
