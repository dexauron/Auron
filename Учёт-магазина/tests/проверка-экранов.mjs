...
/* 13. Расхождение видно в форме, «Разобрать» объясняет, план принимает статьи */
{
  ...
  check('и безнал в ящик не подмешан',
    ...
    /Должно быть в ящике\s*55 000/.test(b1),
    'не подмешан', 'ящик 55 000, выручка 100 000');
       ...
       нельзя. Проверяем, что теперь можно. */
{
  ...
  await поле('deposits', '10000');
  await поле('payouts', '10760');
  await поле('collected', '74081');
  await поле('factCash', '67969');
  await page.waitForTimeout(400);

  let б = await коробка();
  check('ФОРМУЛА НА ЭКРАНЕ ДАЁТ 11 738, КАК И НА БУМАГЕ',
    /Должно быть в ящике\s*11 738/.test(б),
    (б.match(/Должно быть в ящике[^\n]*/) || ['нет'])[0], '11 738 ₽');
  ...
  check('называет инкассацию 74 081 → 17 850',
    /Инкассация\s*74 081 ₽ → 17 850 ₽/.test(б),
    (б.match(/Инкассация[^\n]*→[^\n]*/) || ['нет'])[0], '74 081 ₽ → 17 850 ₽');
  check('и факт 67 969 → 11 738',
    /Факт в ящике\s*67 969 ₽ → 11 738 ₽/.test(б),
    (б.match(/Факт в ящике[^\n]*→[^\n]*/) || ['нет'])[0], '67 969 ₽ → 11 738 ₽');
  check('МИНУСОВЫХ ВЫПЛАТ НЕ ПРЕДЛАГАЕТ', !/Выплаты из ящика [^\n]*→ -/.test(б),
    'не предлагает', 'не предлагает');
  check('и не бросает владельца: расхождение можно сохранить как есть',
    /сохраняйте как есть/.test(б), 'сказано', 'сказано');

  /* Правим одно число по подсказке — ящик обязан сойтись */
  await поле('collected', '17850');
  await page.waitForTimeout(400);
  б = await коробка();
  ...
  /* Ящик пересчитали до инкассации — программа называет это своими словами */
  await поле('collected', '74081');
  await поле('factCash', '85819');
  await page.waitForTimeout(400);
  б = await коробка();
  check('«ПЕРЕСЧИТАЛИ ДО ИНКАССАЦИИ» НАЗВАНО СВОИМИ СЛОВАМИ',
    /до того, как забрали инкассацию/.test(б),
    (б.match(/Похоже[^\n]*/) || ['нет'])[0].slice(0, 60), 'про инкассацию');

  /* Обычная недостача в 200 ₽ — никакой тревоги, это рабочая мелочь */
  await поле('collected', '74081');
  await поле('factCash', '11538');
  await page.waitForTimeout(400);
  б = await коробка();
       ...
       отдельно. Проверяем на его настоящих чеках за 16.09.26. */
{
  ...
  check('В ФОРМЕ ЕСТЬ ОТДЕЛЬНОЕ ПОЛЕ «ИНКАССАЦИЯ: ПЕРЕСЧИТАЛИ»',
    await page.evaluate(() => !!document.querySelector('.sheet [name="collectedFact"]')),
    'есть', 'есть');
  ...
  await поле('deposits', '10000');
  await поле('payouts', '10760');
  await поле('collected', '74081+2688');
  await поле('factCash', '0');
  ...
  check('ТРИ КОЛОНКИ НА ЭКРАНЕ',
    ...
    'все три', 'все три');
  check('пока инкассацию не пересчитали — так и написано',
    /не считали/.test(б), 'написано', 'написано');
  check('и программа не выдумывает недостачу',
    /Всё сошлось/.test(б), (б.match(/ВСЕГО[^\n]*|Всё сошлось/) || ['нет'])[0], 'Всё сошлось');

  /* Пересчитали — и пропажа по дороге видна отдельной строкой */
  await поле('collectedFact', '67969');
  await page.waitForTimeout(500);
  б = await коробка();
  check('ПЕРЕСЧИТАЛИ — ВИДНО, ЧТО ДО СЕЙФА НЕ ДОЕХАЛО 8 800',
    /не хватает 8 800/.test(б), (б.match(/не хватает[^\n]*/) || ['нет'])[0], 'не хватает 8 800 ₽');
  check('ЯЩИК ПРИ ЭТОМ СХОДИТСЯ — пропажа не свалена на него',
    /Должно быть в ящике\s*0 ₽/.test(б) && /Сходится/.test(б), 'сходится', 'сходится');
  ...
  /* Главное: в сейф обязана уехать ПЕРЕСЧИТАННАЯ сумма, а не пробитая.
  ...
  const итог = await page.evaluate(() => {
    const S = window.WMStore, E = window.WM;
    const пер = (S.state.dds || []).filter(r => E.isMove(r) && E.txt(r.category) === 'Инкассация');
    const см = (S.state.dds || []).filter(r => E.isShift(r))[0];
    const c = см ? E.shiftCalc(см) : null;
    return { переводов: пер.length, сумма: пер[0] ? E.num(пер[0].amount) : 0,
      пробито: c ? c.collected : 0, доехало: c ? c.collectedFact : 0,
      пропажа: c ? c.collectShort : 0 };
  });
  check('В СЕЙФ УШЛА ПЕРЕСЧИТАННАЯ СУММА, А НЕ ПРОБИТАЯ КАССОЙ',
    итог.переводов === 1 && итог.сумма === 67969,
    итог.сумма + ' (пробито ' + итог.пробито + ')', '67 969');
