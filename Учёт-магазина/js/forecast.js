...
(function (root, factory) {
  ...
  function taxCalendar(settings, year, taxAmountFn, income, expense) {
    ...
    if (mode.indexOf('усн') >= 0) {
      ...
      /* Стоимость патента владелец вписывает либо за год, либо в месяц —
         второе он и так вводит для расчёта налога. Раньше здесь читалась
         только годовая сумма, поля для неё не было, и календарь всегда
         показывал «Патент: 0 ₽». Теперь берём годовую, а если её нет —
         считаем из месячной. */
      var patentYear = num(settings.patentSum) || num(settings.patentMonth) * 12;
      add(y + '-04-01', 'Патент: первая треть', patentYear / 3, 'если патент на год');
      add(y + '-12-31', 'Патент: остаток', patentYear / 3 * 2, '');
    }
    if (isIP) {
