...
(function () {
  ...
  function shiftSumBox(v) {
    ...
    if (acc && acc.kind !== 'till') {
      h += '<div class="cc-sub" style="color:var(--orange)">⚠ Наличные уходят на счёт «' +
        esc(acc.name) + '», а это не денежный ящик.' +
        u.more('Тогда в ящике так и останется ноль, и сверять будет нечего. ' +
          'Обычно здесь ставят кассу.') + '</div>';
    }
    return h + '</div>';
  }
