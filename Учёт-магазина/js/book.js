  ...
  var SHEETS = [
    ...
    { name: 'ДДС_Операции', coll: 'dds', edit: true, not: 'Смена',
      about: 'Итоги дня (товар за наличные, погашение и новый долг), приходы, ' +
        ...
        'Колонка «Откуда_деньги» решает, трогать ли ящик: «Из ящика» значит, ' +
        'что деньги уже посчитаны в выплатах смены (или в её факте, если это приход), ' +
        'и второй раз их не считают. «Из сейфа» двигает сейф.',
      cols: [['ID', 'id'], ['Тип', 'type'], ['Дата', 'date', 'date'],
        ...
        ['Счёт', 'account'], ['Счёт_куда', 'toAccount'],
        ['Откуда_деньги', 'source'], ['Кто', 'cashier'], ['Комментарий', 'note']] },

    { name: 'Счета', coll: 'accounts', edit: true,
      about: 'Где лежат деньги: денежные ящики, сейф, расчётный счёт, карта. ' +
        'Вид «till» — ящик в зале, его остаток правит сверка смены; «cash» — ' +
        'прочие наличные; «bank» — счёт или карта, туда падает эквайринг и СБП.',
      cols: [['ID', 'id'], ['Название', 'name'], ['Вид', 'kind'],
        ['Было_на_старте', 'opening', 'num'],
        ['Сюда_наличная_выручка', 'defaultCash', 'bool'],
        ['Сюда_безнал', 'defaultCashless', 'bool'],
        ['Убран', 'archived', 'bool'], ['Заметка', 'note']] },
    ...
    { name: 'План_Выплат', coll: 'plans', edit: true,
      ...
      cols: [['ID', 'id'], ['Дата_выплаты', 'due', 'date'],
        ['Что_платим', 'category'], ['Кому', 'supplier'],
        ['Сумма', 'amount', 'num'], ['Статус', 'status'],
        ['Чем_платим', 'method'], ['Повторять', 'repeat'],
        ['Оплачено_когда', 'paidAt', 'date'], ['Комментарий', 'note']] },

    { name: 'Сотрудники', coll: 'staff', edit: true,
      about: 'Кто работает: ставки за день и за ночь, оклад, телефон, когда принят и уволен. ' +
        'Ночная ставка своя — ночь дороже. Уволенный не предлагается в формах, ' +
        'но его смены остаются в отчётах.',
      cols: [['ID', 'id'], ['Имя', 'name'], ['Должность', 'position'],
        ['Как_считаем', 'scheme'],
        ['Ставка_за_час', 'rate', 'num'], ['Ставка_ночь', 'rateNight', 'num'],
        ['Оклад_за_месяц', 'salary', 'num'], ['Процент_с_выручки', 'percent', 'num'],
        ['Норма_смен', 'normShifts', 'num'],
    ...
    { name: 'Выплаты_Зарплаты', coll: 'payouts', edit: true,
      ...
      cols: [['ID', 'id'], ['Дата', 'date', 'date'], ['Сотрудник', 'employee'],
        ['Вид', 'kind'], ['Сумма', 'amount', 'num'], ['Чем', 'method'],
        ['Откуда_деньги', 'source'], ['Комментарий', 'note']] },
