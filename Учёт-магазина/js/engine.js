...
(function (root, factory) {
  ...
  /* --------------------------------------------------------------------------
     ...
     -------------------------------------------------------------------------- */
  /* Поля, каждое из которых могло быть вписано неверно. k — с каким знаком
     поле входит в расчёт: по нему считается, каким оно должно было быть,
     чтобы всё сошлось. k = 0 значит «это и есть итог». */
  var ПОЛЯ_ЯЩИКА = [
    /* k — знак поля в «должны отдать». «Получил» стоит по другую сторону
       равенства, поэтому знак у него обратный: чтобы недостача исчезла,
       получить надо было БОЛЬШЕ, а не меньше. Сначала я написал здесь +1 и
       программа советовала уменьшить полученное — совет ровно наоборот. */
    { key: 'received', k: -1, name: 'Получил на руки', новое: true },
    { key: 'payouts', k: -1, name: 'Кассир платил из ящика' },
    { key: 'kept', k: -1, name: 'Размен оставил в ящике', новое: true },
    { key: 'deposits', k: 1, name: 'Довозили размен' },
    { key: 'returnsCash', k: -1, name: 'Возвраты покупателям' },
    { key: 'zCash', k: 1, name: 'Касса сказала наличными' },
    { key: 'openCash', k: 1, name: 'Размен был на начало' }
  ];

  /* Те же поля, но как их называла программа до перехода на взгляд
     бухгалтера. Старые смены правятся старыми именами — иначе владелец
     искал бы в форме поле, которого там нет. */
  var ПОЛЯ_ЯЩИКА_СТАРЫЕ = [
  ...
  function shiftFix(s) {
    ...
    if (!c.factFilled && !isZero(c.expected)) {
      out.reason = c.recvFilled || !isZero(c.kept)
        ? 'Похоже, вы не вписали, сколько денег получили на руки.'
        : 'Похоже, вы не вписали, сколько денег пересчитали в ящике.';
    } else if (!isZero(c.kept) && same(diff, c.kept)) {
      /* Взгляд бухгалтера: он посчитал деньги ВМЕСТЕ с разменом, который
         собирался оставить кассиру, — и расхождение вышло ровно в размен. */
      out.reason = 'Похоже, вы посчитали деньги вместе с разменом, который оставили ' +
        'кассиру: лишние как раз ' + fmtMoney(c.kept) + '.';
      ...
      out.reason = c.recvFilled
        ? 'Похоже, выплаты записали, а денег из ящика кассир не выдавал.'
        : 'Похоже, выплаты записали, а деньги из ящика не выдали.';
    } else if (!isZero(c.deposits) && same(diff, c.deposits)) {
      out.reason = 'Похоже, довезённый размен посчитали дважды.';
    } else if (!isZero(c.openCash) && same(diff, c.openCash)) {
      out.reason = 'Похоже, размен посчитали дважды: он уже лежал в ящике с утра.';
    } else if (!isZero(c.collected) && same(diff, -c.collected)) {
      out.reason = 'Похоже, инкассацию записали дважды.';
    }

    (c.recvFilled ? ПОЛЯ_ЯЩИКА : ПОЛЯ_ЯЩИКА_СТАРЫЕ).forEach(function (f) {
    ...
    (rows || []).forEach(function (r) {
      ...
      if (isShift(r)) {
        ...
        if (c.recvFilled) {
          ...
          /* Довезённый среди смены размен владелец взял из своего же сейфа.
             Вечером он возвращается к нему внутри забранного, поэтому сейф
             меняется на разницу. Не вычесть — и в сейфе будут числиться
             деньги, которые оттуда уже вынули. */
          hit(txt(r.toAccount) ? accountOf(r, accounts, 'to') : cashAccount(accounts),
            safeRound(c.received - c.deposits));
        } else {
  ...
  function cashBook(rows, settings, from, to, accounts) {
    ...
    (rows || []).forEach(function (r) {
      ...
      /* «ИТОГИ ДНЯ» В КНИГУ НЕ ПОПАДАЮТ, И ЭТО НЕ ЗАБЫВЧИВОСТЬ.

         Сначала я вычитал отсюда закуп и погашение долга — выглядело
         логично: деньги же ушли. Правило на тысяче магазинов поймало это
         сразу: книга разошлась с остатком наличных на 982 магазинах из
         1000, на первом же — на 235 тысяч.

         Причина простая. «Итоги дня» — это учёт ДОЛГА поставщикам, а не
         движение денег: остатки по счетам эта форма не трогает намеренно,
         потому что сами деньги уже ушли через «выплаты кассира» той смены.
         Вычесть их ещё и в книге значит потерять одни и те же рубли дважды.

         Заплатили поставщику из своего сейфа — это обычный расход, и он
         попадёт в книгу строкой выше, как все расходы.

         Книга обязана сходиться с остатком наличных до копейки: иначе
         владелец видит два числа и не знает, какому верить. */
      } else if (isMove(r)) {
