...
(function () {
  ...
  /* СПИСАНИЯ ИЗ 1С ЗА МЕСЯЦ.

     Здесь было две беды сразу. Строки списаний не имеют поля date — у них
     период выгрузки, from и to. Прежний отбор «если есть date и месяц не
     тот — пропустить» не срабатывал никогда, и ВЕСЬ файл списаний попадал в
     прибыль КАЖДОГО месяца. Тихо: числа выглядели правдоподобно.

     Вторая — про настройку «Списания 1С приводить к месяцу», которая не
     читалась. Выгрузка редко ложится ровно на месяц: попросили за 45 дней —
     и месячная прибыль искажается периодом выгрузки, а не делами магазина.

        «да»  — берём ту долю периода, что попала в месяц (по дням);
        «нет» — берём весь файл целиком, если он месяца касается.

     Долю считаем по дням, а не поровну: выгрузка за 45 дней, из которых в
     месяц попало 20, даёт 20/45 суммы. */
  function writeoff1c(m) {
    var d = D(), месяц = m || ym();
    var кМесяцу = E.norm(S.settings.writeoffsToMonth) !== 'нет';
    var мОт = месяц + '-01';
    var мДо = новыйДень(месяц + '-01', дней(месяц) - 1);
    var sum = 0;
    (d.writeoffs || []).forEach(function (r) {
      var от = E.txt(r.from), до = E.txt(r.to) || от;
      // у строки нет периода — судим по дате, как раньше
      if (!от) {
        if (!r.date || String(r.date).slice(0, 7) === месяц) sum += num(r.cost);
        return;
      }
      if (до < мОт || от > мДо) return;               // период мимо месяца
      if (!кМесяцу) { sum += num(r.cost); return; }
      var всего = E.daysBetween(от, до) + 1;
      var пересОт = от > мОт ? от : мОт;
      var пересДо = до < мДо ? до : мДо;
      var внутри = E.daysBetween(пересОт, пересДо) + 1;
      sum += num(r.cost) * (всего > 0 ? внутри / всего : 1);
    });
  ...
  function новыйДень(iso, плюс) {
    var d = new Date(iso + 'T00:00:00');
    d.setDate(d.getDate() + плюс);
  ...
  function viewEarners() {
    ...
    if (c.byGroup && c.byGroup.length) {
      ...
      /* Раньше здесь стояло .slice(0, 30) поверх постраничного показа: тридцать
         первая группа просто исчезала, и владелец об этом не узнавал. Таблица
         и так показывает по тридцать строк с кнопкой «показать ещё» — обрезать
         сверх этого значит прятать. */
      ], gp.rows, { step: 30 }),
