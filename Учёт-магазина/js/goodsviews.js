...
(function () {
  ...
  /* --- Поставщики и общий долг ----------------------------------------------------------
     Владелец: «Начать просто, углубляться по желанию». Сверху — одна цифра,
     та самая, ради которой он сюда и зашёл: сколько всего должен. Кнопка
     «Разбить по поставщикам» раскрывает подробности на этом же экране, без
     перехода: так видно и общее, и частное разом.

     ЧТО ИМЕННО РАЗБИВАЕТСЯ. Общий долг магазина — ручная цифра из вечерних
     итогов, имён в ней нет, и разложить её по поставщикам невозможно. Зато
     выплаты владелец заводит с именем, а накладные приходят из 1С с именем —
     вот это и раскладывается. Подписано прямо, чтобы одно не приняли за
     другое.

     Раскрыто или свёрнуто — помнится на время работы программы. А у того,
     кто в настройках сказал «долг веду по накладным», экран сразу открыт:
     он и пришёл сюда за подробностями. */
  var РАЗБИТЬ = null;      // null — как сказано в настройках, дальше решает владелец
  var РАЗБИТЬ_ПРИ = '';    // каким было «долг веду», когда он решил

  function поНакладным() { return E.txt(S.settings.debtMode) === 'по накладным'; }

  function виднаРазбивка() {
    /* Настройку поменяли после того, как он свернул или раскрыл руками —
       значит, сказано новее и яснее: слушаемся настройку. Без этого
       переключатель в «Настройках» на этом экране ничего бы не менял, а
       это ровно та беда, из-за которой настройка и была мёртвой. */
    if (РАЗБИТЬ !== null && РАЗБИТЬ_ПРИ === E.txt(S.settings.debtMode)) return РАЗБИТЬ;
    РАЗБИТЬ = null;
    return поНакладным();
  }
  ...
  function viewSuppliers() {
    ...
    var подробно = виднаРазбивка();

    var h = u.pageHead('Поставщики и долг', 'Сколько магазин должен и кому платить ближайшие дни',
      '<button class="btn btn-primary" data-form="payPlan">' + ic('plus') + ' Запланировать выплату</button>');

    /* ОДНА ЦИФРА, ЗА КОТОРОЙ ПРИШЛИ. Остальное — по кнопке. */
    h += u.hero('Должны поставщикам', u.priv(debt.debt), 'общей суммой по магазину',
      debt.debt >= num(S.settings.debtCrit) ? 'c-red' : '');

    if (pt.overdue) {
      h += '<div class="banner"><span>' + ic('warning') + '</span><span>Просрочено выплат на ' +
        '<b class="private">' + money(pt.overdue) + '</b> — ' + pt.overdueCount + ' ' +
        E.plural(pt.overdueCount, 'платёж', 'платежа', 'платежей') +
        '. <a href="#" data-go="finpay">Перейти к выплатам</a></span></div>';
    }

    h += '<div class="row-btns"><button class="btn" data-act="sup-split">' +
      ic(подробно ? 'chevronUp' : 'chevronDown') + ' ' +
      (подробно ? 'Свернуть подробности' : 'Разбить по поставщикам') + '</button></div>';

    if (!подробно) return h;

    /* --- Дальше — только для тех, кто нажал ---------------------------------- */

    h += '<div class="stat-grid">' +
      u.stat('Взято в долг', u.priv(debt.taken), 'за всё время') +
      u.stat('Погашено', u.priv(debt.paid), 'за всё время', 'c-green') +
      u.stat('Запланировано выплат', u.priv(pt.planned), pt.plannedCount + ' ' +
        E.plural(pt.plannedCount, 'платёж', 'платежа', 'платежей')) +
      u.stat('Просрочено', u.priv(pt.overdue), pt.overdueCount + ' ' +
        E.plural(pt.overdueCount, 'платёж', 'платежа', 'платежей'),
        pt.overdue ? 'c-red' : 'c-green') +
      '</div>';

    var привоз = c.supplies ? (c.supplies.rows || c.supplies) : [];
    var разбивка = E.supplierBreakdown(S.state.plans || [], привоз, E.today());

    var cols = [
      { title: 'Поставщик', fn: function (r) { return esc(r.supplier); } },
      { title: 'К выплате', cls: 'num', fn: function (r) {
        return r.planned ? u.priv(r.planned) : '<span class="c-muted">—</span>'; } },
      { title: 'Просрочено', cls: 'num', fn: function (r) {
        return r.overdue ? '<span class="c-red private">' + money(r.overdue) + '</span>'
          : '<span class="c-muted">—</span>'; } },
      { title: 'Ближайшие дни', cls: 'num', fn: function (r) {
        return r.soon ? u.priv(r.soon) : '<span class="c-muted">—</span>'; } },
      { title: 'Телефон', fn: function (r) {
        var ph = c.contactsIdx ? c.contactsIdx[E.norm(r.supplier)] : '';
        return ph ? '<a href="tel:' + esc(ph) + '">' + esc(ph) + '</a>'
          : '<span class="c-muted">—</span>'; } }
    ];
    /* Столбцы про привоз показываем, только если накладные 1С загружены.
       Пустой столбец из прочерков — это место, занятое ничем. */
    if (привоз.length) {
      cols.splice(4, 0,
        { title: 'Привезли', cls: 'num', fn: function (r) {
          return r.brought ? u.priv(r.brought) : '<span class="c-muted">—</span>'; } },
        { title: 'Оплачено по ордерам', cls: 'num', fn: function (r) {
          return r.paidDocs ? u.priv(r.paidDocs) : '<span class="c-muted">—</span>'; } });
    }

    h += u.card('По поставщикам', u.table('supT', cols, разбивка,
      { step: 40, empty: 'Ни одной выплаты с именем поставщика пока не заведено' }),
      'Это выплаты и привоз по именам — не разложенный общий долг');

    h += '<div class="banner blue"><span>' + ic('clipboard') + '</span><span>' +
      '<b>Почему суммы в таблице не складываются в общий долг.</b> Общий долг — ' +
      'ручная цифра из вечерних итогов: вечером вы вписываете, сколько взяли в долг и ' +
      'сколько погасили, без имён. В таблице — то, что программа знает по именам: ' +
      'ваши запланированные выплаты' +
      (привоз.length ? ' и накладные из 1С' : '') + '. Это разные вещи, и сходиться ' +
      'они не обязаны.</span></div>';

    if (!привоз.length) {
      h += '<div class="banner"><span>' + ic('info') + '</span><span>Если загрузить отчёт ' +
        '«Приходные накладные» из 1С на экране «Данные и копии», в таблице добавятся ' +
        'столбцы «Привезли» и «Оплачено по ордерам». Без выгрузки долг это не меняет.</span></div>';
    }
    return h;
  }
  ...
  /* «Разбить по поставщикам» / «Свернуть подробности». Помним на время
     работы программы: открыл один раз — при следующем заходе на экран
     подробности на месте, перезапустил — снова просто, как он и просил
     («начать просто, углубляться по желанию»). */
  A['sup-split'] = function () {
    РАЗБИТЬ = !виднаРазбивка();
    РАЗБИТЬ_ПРИ = E.txt(S.settings.debtMode);
