...
(function () {
  ...
  function applyLook() {
    ...
    /* Плотность. Компактно — меньше отступов, на экран помещается больше.
       Пустая настройка у старых баз означает «компактно»: это то, что
       владелец попросил, и то, что стоит по умолчанию. */
    document.body.classList.toggle('compact', E.norm(s.density || 'компактно') !== 'просторно');
    рисоватьКнопкуТемы();
  }

  /* --- ПЕРЕКЛЮЧАТЕЛЬ ТЕМЫ В ШАПКЕ ----------------------------------------
     Светлая тема в программе была и раньше, но лежала в «Настройках» на
     седьмом экране прокрутки. Владелец про неё просто не знал и попросил
     «чтобы была и белая». Значит, дело не в теме, а в том, что до неё не
     добраться. Одна кнопка рядом с «глазом»: солнце — включить светлую,
     месяц — тёмную.

     Нажатие ставит тему ЯВНО, а не переключает «авто»: человек нажал на
     солнце — он хочет светлую сейчас и завтра, а не «как в системе». */
  function сейчасТёмная() {
    var явно = document.documentElement.getAttribute('data-theme');
    if (явно) return явно === 'dark';
    try { return window.matchMedia('(prefers-color-scheme: dark)').matches; }
    catch (e) { return false; }
  }

  function рисоватьКнопкуТемы() {
    var b = $('themeBtn'); if (!b) return;
    var тёмная = сейчасТёмная();
    b.innerHTML = ic(тёмная ? 'sun' : 'moon', 20);
    b.title = тёмная ? 'Включить светлую тему' : 'Включить тёмную тему';
  }

  function перевернутьТему() {
    S.setSetting('theme', сейчасТёмная() ? 'Светлая' : 'Тёмная');
    applyLook();
    render();
    toast(сейчасТёмная() ? 'Тёмная тема' : 'Светлая тема', 2200);
  }
  ...
  function viewData() {
    ...
    /* --- ПРИМЕР ЗА ГОД ----------------------------------------------------
       Пустая программа ничего не говорит о себе: непонятно, что считается,
       как выглядят отчёты, что вообще можно спрашивать. Кнопка заполняет её
       годом работы обычного магазина — и убирает этот год одним нажатием.

       Каждая запись примера помечена, и «Убрать пример» удаляет ровно их.
       Без метки пришлось бы чистить по дате или по «похожести» — это способ
       однажды стереть владельцу его работу. */
    var DEMO = window.WMDemo;
    if (DEMO) {
      var примеров = DEMO.счёт(S, true), своих = DEMO.счёт(S, false);
      h += card('Посмотреть на заполненной программе', listOf([
        listRow({ icon: 'calendar',
          title: примеров ? 'Пример за год уже загружен' : 'Заполнить примером за год',
          sub: примеров
            ? nf(примеров) + ' ' + plural(примеров, 'запись', 'записи', 'записей') +
              ' примера. Ваши записи не тронуты'
            : 'смены, расходы, зарплата, долги, товар — всё как у работающего магазина',
          value: примеров ? ''
            : '<button class="btn btn-sm btn-primary" data-act="demo-fill">Заполнить</button>' }),
        listRow({ icon: 'trash', title: 'Убрать пример',
          sub: примеров ? 'удалит только записи примера' : 'примера в базе нет',
          value: примеров
            ? '<button class="btn btn-sm btn-danger" data-act="demo-clear">Убрать</button>' : '' })
      ], ''), своих
        ? 'В базе ' + nf(своих) + ' ваших ' + plural(своих, 'запись', 'записи', 'записей')
        : 'База пока пуста');
    }
    return h;
  }
  ...
  /* Экраны, которые нужны только тем, кто ведёт табель. Магазину, где
     зарплату «ведут просто», табель и график смен в меню не нужны: они
     будут стоять пустыми и сбивать с толку. Экран не удалён — он
     открывается поиском и палитрой, как любой спрятанный. */
  var ТОЛЬКО_С_ТАБЕЛЕМ = { timesheet: 1, sched: 1 };

  function подУровнем(v) {
    if (!v || isFav(v.id)) return false;
    /* «Показывать в меню все экраны» сильнее уровня подробности: владелец
       прямо сказал «покажи всё», и прятать после этого — спорить с ним. */
    if (E.norm(S.settings.showAllViews) === 'да') return false;
    if (ТОЛЬКО_С_ТАБЕЛЕМ[v.id] &&
        E.norm(S.settings.salaryMode).indexOf('табел') < 0) return true;
  ...
  function bind() {
    document.addEventListener('click', function (e) {
      ...
      /* Скрыть экран совсем. Не удаление: он остаётся в «Настроить меню»,
        ...
        var DEMO2 = window.WMDemo;
        if (!DEMO2) return;
        var своих2 = DEMO2.счёт(S, false);
        var вопрос = своих2
          ? 'В базе уже есть ' + nf(своих2) + ' ваших записей. Пример добавится К НИМ, ' +
            'а не вместо них — и убрать его можно будет одной кнопкой. Заполнить?'
          : 'Заполнить программу примером за год? Это записи выдуманного магазина — ' +
            'посмотреть, как всё выглядит. Убрать их можно одной кнопкой.';
        if (!confirm(вопрос)) return;
        /* Копию делаем ДО, а не после: если владелец передумает, а кнопка
           «Убрать» вдруг не сработает, у него всё равно останется файл. */
        if (своих2) backup();
        var n1 = DEMO2.заполнить(S, E, D, 365);
        DEMO2.товары(D, E, 365);
        recompute(); render();
        toast('Готово: ' + nf(n1) + ' ' + plural(n1, 'запись', 'записи', 'записей') +
          ' за год. Походите по экранам — теперь они заполнены. ' +
          'Убрать пример: «Данные и копии» → «Убрать пример».', 11000);
      }
      else if (a === 'demo-clear') {
        var DEMO3 = window.WMDemo;
        if (!DEMO3) return;
        if (!confirm('Убрать пример? Ваши записи останутся на месте.')) return;
        var n2 = DEMO3.убрать(S);
        D.sales = []; D.stock = []; D.prices = []; D.contacts = [];
        D.writeoffs = []; D.dead = []; D.returns = []; D.files = [];
        recompute(); render();
    ...
    /* НОВЫЕ ВЫГРУЗКИ ПОДХВАТЫВАЮТСЯ САМИ. Владелец кладёт свежий отчёт из
       1С в папку и ждёт, что программа его увидит, — настройка «Проверять
       папку каждые, секунд» ровно это и обещала, ничего при этом не делая.

       Это дёшево: listExports() читает только имена, размеры и время файлов,
       а syncFolder берёт содержимое ТОЛЬКО у изменившихся — остальные
       отсекаются по отпечатку. Когда в папке ничего не менялось, проверка
       не стоит почти ничего.

       Меньше пяти секунд не ставим: чаще — это работа ради работы. Ноль
       выключает совсем. */
    var каждые = Math.round(num(S.settings.autoSyncSeconds));
    if (каждые > 0) {
      setInterval(function () {
        if (F.state !== 'ready') return;
        if (document.getElementById('wmForm')) return;   // не мешаем, пока заполняют форму
        syncFolder(true);
      }, Math.max(5, каждые) * 1000);
    }
