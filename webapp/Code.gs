...
function _ddsCompute(b, rate, debtYesterday, fixedPlan) {
  ...
  // «На закуп» — сколько из дневной выручки реально можно потратить на
  // товар. Из торговли уходит: наценка, плановая прибыль, выданное из
  // кассы, перевод, взятое хозяином, расходы дня и постоянные.
  //
  // Постоянные (зарплата, аренда, коммунальные, налог) вычитаются
  // ПОЛНОСТЬЮ — владелец: «это отложенные деньги». В его таблице они
  // возвращались обратно (формула вычитает прибыль, а прибыль их уже
  // вычла — минус на минус), и «на закуп» за июль выходил больше
  // настоящего на 1 302 000 ₽ постоянных плюс 4 500 ₽ ГСМ и 166 301 ₽
  // ночного Имана — 1 472 801 ₽ из 8 076 383, то есть 18%.
  //
  // Иман и ГСМ считаются одинаково в обе смены: в таблице дневной Иман
  // вычитался, а ночной возвращался, и ГСМ возвращался, хотя обед,
  // списание, комиссия и расходники — нет. Делового смысла в этой
  // разнице нет, это описка формулы.
  //
  // Если раскрыть скобки, всё сходится к простому:
  //   на закуп = общая торговля − 50% − выплата кассы.
  var forBuy = trade - gross - profitPlan - b.cashOut - b.transfer - b.iman
               - daily - fixed;
  return {
