...
// ── «На закуп»: наценка вычитается ОДИН раз ─────────────────────────
// В таблице владельца она вычиталась дважды — прямо и внутри прибыли.
// За июль «можно потратить» выходило 8 076 383 ₽ при себестоимости
// проданного 9 905 373 и фактических тратах на товар 9 512 067.
// По такой цифре закуп был бы меньше продаж, и товар на полках таял бы.
//
// Оставшиеся 75% — это и есть себестоимость проданного: сколько ушло с
// полок, столько надо докупить. Расходы дня, Иман и постоянные здесь не
// вычитаются — они платятся из наценки, а наценка уже вынута.
var fb=_ddsCompute(box({cashRev:100000}),0.25,0,20000);
t('на закуп = торговля − наценка', fb.forBuy===75000, fb.forBuy);
t('постоянные не вычитаются второй раз',
  _ddsCompute(box({cashRev:100000}),0.25,0,0).forBuy===fb.forBuy);
t('расходы дня не уменьшают «на закуп»',
  _ddsCompute(box({cashRev:100000,meal:5000,fuel:900,writeOff:300}),0.25,0,0).forBuy===75000);
t('Иман не уменьшает «на закуп»',
  _ddsCompute(box({cashRev:100000,iman:7000}),0.25,0,0).forBuy===75000);
t('выданное из кассы уменьшает: денег в ящике нет',
  _ddsCompute(box({cashRev:100000,cashOut:10000}),0.25,0,0).forBuy===65000);
t('безнал тоже считается торговлей',
  _ddsCompute(box({cashRev:60000,onlineRev:40000}),0.25,0,0).forBuy===75000);
// Настоящий июль владельца.
var jul=_ddsCompute(box({cashRev:8025186,onlineRev:5181978}),0.25,0,1302000);
t('июль: доступно 9 905 373 ₽', jul.forBuy===9905373, jul.forBuy);
t('это больше, чем потрачено на товар (9 512 067)', jul.forBuy>9512067);
// Постоянные по-прежнему уменьшают ПРИБЫЛЬ — они отложены внутри наценки.
t('постоянные уменьшают прибыль',
  _ddsCompute(box({cashRev:100000}),0.25,0,20000).profitPlan===5000);
