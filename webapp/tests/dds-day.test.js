...
// ── «На закуп»: постоянные расходы — отложенные деньги ──────────────
// Правило владельца. В его таблице формула вычитала прибыль, а прибыль
// постоянные уже вычла — минус на минус, и они возвращались в сумму
// «можно потратить». За июль так набегало 1 472 801 ₽ лишних из
// 8 076 383 — 18%. Тестов на эту колонку не было вовсе, поэтому
// расхождение жило незамеченным.
var fb=_ddsCompute(box({cashRev:100000,salary:0}),0.25,0,20000);
t('постоянные НЕ возвращаются в «на закуп»',
  fb.forBuy===100000-25000-25000, fb.forBuy);
var fb2=_ddsCompute(box({cashRev:100000}),0.25,0,0);
t('без плана результат тот же (нечего вычитать)', fb2.forBuy===50000, fb2.forBuy);
t('план уменьшает прибыль, но не «на закуп»',
  fb.profitPlan===fb2.profitPlan-20000 && fb.forBuy===fb2.forBuy);

// Иман и ГСМ — одинаково в обе смены. В таблице дневной Иман вычитался,
// ночной возвращался, ГСМ возвращался, а обед и списание — нет.
var im=_ddsCompute(box({cashRev:100000,iman:7000}),0.25,0,0);
t('Иман не влияет на «на закуп» ни в одну сторону', im.forBuy===50000, im.forBuy);
var gs1=_ddsCompute(box({cashRev:100000,fuel:5000}),0.25,0,0);
var gs2=_ddsCompute(box({cashRev:100000,meal:5000}),0.25,0,0);
t('ГСМ и обед считаются одинаково', gs1.forBuy===gs2.forBuy, gs1.forBuy+' vs '+gs2.forBuy);
// Выданное из кассы уменьшает доступное.
var co=_ddsCompute(box({cashRev:100000,cashOut:10000}),0.25,0,0);
t('выплата кассы уменьшает «на закуп»', co.forBuy===40000, co.forBuy);
// Свёрнутая форма: торговля − 50% − выплата кассы.
t('сходится к «торговля − 50% − выплата кассы»',
  _ddsCompute(box({cashRev:200000,onlineRev:80000,cashOut:30000,iman:9000,
    meal:1000,fuel:500,writeOff:200}),0.25,0,42000).forBuy === 280000-140000-30000);
