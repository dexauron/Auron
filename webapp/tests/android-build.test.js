...
// ── Мины, на которых сборка падала ───────────────────────────────────
// Каждая проверка ниже поставлена после настоящего падения на GitHub.
t('библиотека для картинок ставится — без неё значок не собрать',
  /pip install .*pillow/.test(wf));
t('Java 17 — под неё сделан Capacitor 6',
  /java-version: '17'/.test(wf));
t('«живой» значок Capacitor убирается — иначе телефон покажет его, а не наш',
  /rm -rf "\$R\/mipmap-anydpi-v26"/.test(wf));
t('Gradle получает память явно — иначе падает без внятной причины',
  /org\.gradle\.jvmargs=-Xmx3g/.test(wf));
t('ошибка создания проекта не прячется',
  !/npx cap add android \|\| echo/.test(wf));
t('проверяем, что проект создался', /test -f android\/gradlew/.test(wf));
t('проверяем, что значки на месте', /Нет значка \$d/.test(wf));
