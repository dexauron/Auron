...
// Наценка считается ПО ДЕНЬГАМ, а не средним по товарам:
// продано только Молоко — выручка 1000, прибыль 200 → себестоимость 800,
// наценка 200/800 = 25%. Крупа не продавалась и на наценку не влияет.
// (Простое среднее дало бы 26.7%, учитывая непроданную Крупу — это врало.)
eq('товары: наценка по деньгам 200/800', ga.avgMarkup, 25);
eq('товары: простое среднее сохранено отдельно', ga.avgMarkupSimple, 26.7);
...
// ── Auron-советник: охотник за экономией ────────────────────────────
...
// ── Auron-советник: проактивные подсказки ───────────────────────────
...
// ── Auron-помощник: локальные ответы по данным (без ключа ИИ) ────────
...
// ── Общий долг = сумма долгов всех поставщиков ──────────────────────
// С 5.9.0 «долг магазина» — это не отдельный счётчик, а сумма долгов
// всем поставщикам. Сверка по факту приводит к цели ОБЩУЮ сумму, а
// разницу вешает на служебного контрагента: неизвестно, чей именно долг
// посчитан неверно, и раскидывать её по живым поставщикам нельзя.
const alldebts=A.getDebts({ssId:'ss1'});
const storeRow=alldebts.find(d=>String(d.name).indexOf('🏪')===0);
eq('служебная строка поправок есть в getDebts', !!storeRow, true);
eq('сверка привела ОБЩИЙ долг к 100000', A.getStoreDebt({ssId:'ss1'}).debt, 100000);
eq('на служебной строке лежит только разница', storeRow&&storeRow.debt, 97000);
eq('долги живых поставщиков сверка не тронула',
   alldebts.filter(d=>String(d.name).indexOf('🏪')!==0)
           .reduce((s,d)=>s+Math.max(d.debt||0,0),0), 3000);
...
// ── Наценка не должна ломаться от одной битой цены ──────────────────
// Реальный случай из выгрузки владельца: товар с закупкой 0.09 ₽ при
// продаже 75 ₽ давал наценку 83 000% и задирал среднее по магазину
// со ~30% до 193%. Взвешенная по деньгам наценка так ломаться не должна.
A.saveGoods({ssId:'ss2',kind:'Цены',rows:[
  {barcode:'A',name:'Обычный товар',buy:100},
  {barcode:'B',name:'Битая цена',   buy:0.09}]});
A.saveGoods({ssId:'ss2',kind:'Продажи',salesDays:30,rows:[
  {barcode:'A',name:'Обычный товар',qty:100,revenue:13000,profit:3000,retail:130},
  {barcode:'B',name:'Битая цена',   qty:1,  revenue:75,   profit:74.9,retail:75}]});
const gb=A.getGoodsAnalytics({ssId:'ss2'});
// по деньгам: выручка 13075, прибыль 3074.9 → себестоимость 10000.1 → ~30.7%
ok('наценка по деньгам не улетает от битой цены', gb.avgMarkup>25 && gb.avgMarkup<35,
   'получено '+gb.avgMarkup+'%');
ok('простое среднее действительно улетает', gb.avgMarkupSimple>1000,
   'получено '+gb.avgMarkupSimple+'%');
ok('битая цена попала в список на исправление',
   (gb.priceIssues||[]).some(function(x){return x.name==='Битая цена';}));

// ── Рентабельность ≠ наценка ────────────────────────────────────────
// Наценка = прибыль / СЕБЕСТОИМОСТЬ (сколько накинули сверху).
// Рентабельность = прибыль / ВЫРУЧКА (сколько осталось с рубля продаж).
// Проверяем математическую связь, а не конкретные числа: в этом тесте
// один общий склад на все ssId, данные накапливаются от блока к блоку.
// Связь: выручка = себестоимость + прибыль, отсюда
//        рентабельность = наценка / (1 + наценка).
A.saveGoods({ssId:'ss3',kind:'Цены',rows:[{barcode:'X',name:'Товар',buy:80}]});
A.saveGoods({ssId:'ss3',kind:'Продажи',salesDays:30,rows:[
  {barcode:'X',name:'Товар',qty:100,revenue:10000,profit:2000,retail:100}]});
const gm=A.getGoodsAnalytics({ssId:'ss3'});
ok('рентабельность посчитана', gm.avgMargin!=null && gm.avgMargin>0, 'получено '+gm.avgMargin);
ok('рентабельность НИЖЕ наценки', gm.avgMargin < gm.avgMarkup,
   'наценка '+gm.avgMarkup+'% рент. '+gm.avgMargin+'%');
// рент = нац/(1+нац), допускаем 0.15 п.п. на округление до одного знака
var mk=gm.avgMarkup/100, expect=Math.round(mk/(1+mk)*1000)/10;
ok('связь наценки и рентабельности сходится',
   Math.abs(gm.avgMargin-expect)<=0.15,
   'ожидали ~'+expect+'% получили '+gm.avgMargin+'%');
ok('рентабельность в разумных границах (0..100%)',
   gm.avgMargin>0 && gm.avgMargin<100, 'получено '+gm.avgMargin);
