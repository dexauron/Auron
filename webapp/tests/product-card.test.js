...
// ── Чего не хватало против POS-программ ──────────────────────────────
t('карточкой можно поделиться', /_productShare:function/.test(html));
t('в тексте есть цены и остаток',
  /_productShare[\s\S]{0,900}Закупка: /.test(html) && /_productShare[\s\S]{0,900}Остаток: /.test(html));
t('в тексте есть поставщики', /_productShare[\s\S]{0,1200}Поставщики:/.test(html));
t('пустая карточка не шлётся', /_productShare[\s\S]{0,200}Карточка ещё не загружена/.test(html));
