...
(function (root, factory) {
  ...
  /* Число из ячейки 1С.
     В русской выгрузке разделитель тысяч — пробел, а запятая всегда
     десятичная: «2,500» это 2,5 кг, а не две с половиной тысячи.
     Но файл, пересохранённый в английской раскладке, приходит как
     «1,234.56» — там запятая уже разделяет тысячи. Различаем по тому,
     какой знак стоит правее: он и есть десятичный. Раньше «1,234.56»
     превращалось в 1,234 — ошибка в тысячу раз. */
  function num(v) {
    ...
    if (s.indexOf(',') >= 0 && s.indexOf('.') >= 0) {
      s = s.lastIndexOf(',') > s.lastIndexOf('.')
        ? s.replace(/\./g, '').replace(',', '.')      // 1.234,56 — европейский
        : s.replace(/,/g, '');                        // 1,234.56 — английский
    } else {
      s = s.replace(/,/g, '.');
    }
    var m = s.match(/-?\d+(?:\.\d+)?/);
