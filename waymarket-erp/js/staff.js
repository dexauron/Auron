...
(function (root, factory) {
  ...
  function absencesFor(list, name, from, to) {
    var k = norm(name);
    return (list || []).filter(function (a) {
      if (name && norm(a.employee) !== k) return false;
      if (from && a.to < from) return false;
      if (to && a.from > to) return false;
      return true;
    });
  }
  function absenceDays(a) {
    if (!a || !a.from || !a.to) return 0;
    return Math.max(1, Math.round((new Date(a.to) - new Date(a.from)) / 86400000) + 1);
  }
  // Занят ли человек в этот день
  function busyOn(list, name, date) {
    var k = norm(name);
    return (list || []).filter(function (a) {
      return norm(a.employee) === k && a.from <= date && a.to >= date;
    })[0] || null;
  }

  /* --- 96. Кто был на смене, когда не сошлись деньги ---------------------------- */
  function whoWasThere(state, date, shift) {
    var out = [];
    (state.timesheet || []).forEach(function (r) {
      if (r.date !== date) return;
      if (shift && norm(r.shift) !== norm(shift)) return;
      out.push({ who: r.employee, shift: r.shift, hours: num(r.hours), source: 'табель' });
    });
    (state.dds || []).forEach(function (r) {
      if (r.date !== date || !r.cashier) return;
      if (shift && norm(r.shift) !== norm(shift)) return;
      if (out.some(function (x) { return norm(x.who) === norm(r.cashier); })) return;
      out.push({ who: r.cashier, shift: r.shift, hours: null, source: 'касса' });
    });
    return out;
  }

  // Все случаи, когда деньги не сошлись, с людьми на смене
  function shortageCases(state, minSum) {
    var need = num(minSum) || 1;
    var out = [];
    (state.dds || []).forEach(function (r) {
      var d = num(r.diff);
      if (!d || Math.abs(d) < need) return;
      out.push({ date: r.date, shift: r.shift || '', diff: round(d), kind: 'смена',
        cashier: r.cashier || '', who: whoWasThere(state, r.date, r.shift) });
    });
    (state.cashcount || []).forEach(function (c) {
      var d = num(c.diff);
      if (!d || Math.abs(d) < need) return;
      out.push({ date: c.date, shift: '', diff: round(d), kind: 'пересчёт',
        cashier: c.cashier || '', who: whoWasThere(state, c.date) });
    });
  ...
  function taskStats(tasks) {
    var t = { all: 0, done: 0, open: 0, late: 0 };
    var now = today();
    (tasks || []).forEach(function (x) {
      t.all++;
      if (x.done) t.done++;
      else { t.open++; if (x.due && x.due < now) t.late++; }
    });
    t.share = t.all ? round(t.done / t.all * 100) : 0;
    return t;
  }
  function tasksFor(tasks, name) {
    var k = norm(name);
    return (tasks || []).filter(function (x) { return !name || norm(x.employee) === k; })
      .sort(function (a, b) {
  ...
  return {
    SCHEMES: SCHEMES, ABSENCE: ABSENCE,
    personOf: personOf, accrue: accrue,
    shiftTiming: shiftTiming, timesheetStats: timesheetStats,
    schedule: schedule,
    absencesFor: absencesFor, absenceDays: absenceDays, busyOn: busyOn,
    whoWasThere: whoWasThere, shortageCases: shortageCases,
    taskStats: taskStats, tasksFor: tasksFor,
