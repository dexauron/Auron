...
(function (root, factory) {
  ...
  /* --- 72/73. История цен ----------------------------------------------------
     ...
     ---------------------------------------------------------------------- */
  function snapshot(prices, date) {
    var d = date || today();
    var rows = [];
    (prices || []).forEach(function (p) {
      var price = num(p.price);
      if (!price || !p.key) return;
      rows.push({ key: p.key, name: p.name, supplier: p.supplier || '', price: price });
    });
    return { date: d, rows: rows, count: rows.length };
  }

  function addSnapshot(history, snap) {
    var h = (history || []).filter(function (s) { return s.date !== snap.date; });
    h.push(snap);
    h.sort(function (a, b) { return a.date.localeCompare(b.date); });
    return h.slice(-60);           // хранить полтора года снимков достаточно
  }

  // История одного товара: цена по датам и у кого
  function priceHistory(history, key, supplier) {
    var out = [];
    (history || []).forEach(function (s) {
      (s.rows || []).forEach(function (r) {
        if (r.key !== key) return;
        if (supplier && norm(r.supplier) !== norm(supplier)) return;
        out.push({ date: s.date, price: r.price, supplier: r.supplier, name: r.name });
      });
    });
    out.sort(function (a, b) { return a.date.localeCompare(b.date) || a.supplier.localeCompare(b.supplier); });
    return out;
  }

  // Что подорожало между последними двумя снимками
  function priceJumps(history, minPct) {
    var pct = num(minPct) || 5;
    if (!history || history.length < 2) return { rows: [], from: '', to: '' };
    var prev = history[history.length - 2], last = history[history.length - 1];
    var was = {};
    prev.rows.forEach(function (r) { was[r.key + '|' + norm(r.supplier)] = r; });
    var rows = [];
    last.rows.forEach(function (r) {
      var old = was[r.key + '|' + norm(r.supplier)];
      if (!old || !old.price) return;
      var diff = round(r.price - old.price);
      if (!diff) return;
      var change = round(diff / old.price * 100);
      if (Math.abs(change) < pct) return;
      rows.push({ key: r.key, name: r.name, supplier: r.supplier,
        was: old.price, now: r.price, diff: diff, pct: change, up: diff > 0 });
    });
    rows.sort(function (a, b) { return Math.abs(b.pct) - Math.abs(a.pct); });
    return { rows: rows, from: prev.date, to: last.date,
      up: rows.filter(function (r) { return r.up; }).length,
      down: rows.filter(function (r) { return !r.up; }).length };
  }
  ...
  /* --- 84. Возврат поставщику как документ ------------------------------------ */
  function returnDoc(v) {
    return {
      date: v.date || today(), firm: v.firm || '', doc: v.doc || '',
      reason: v.reason || '', sum: round(num(v.sum)), qty: num(v.qty),
      goods: v.goods || '', basis: v.basis || '', note: v.note || '',
      accepted: !!v.accepted, offset: v.offset !== false
    };
  }
  function returnTotals(rows) {
    var t = { count: 0, sum: 0, waiting: 0, waitingSum: 0 };
    (rows || []).forEach(function (r) {
      t.count++; t.sum += num(r.sum);
      if (!r.accepted) { t.waiting++; t.waitingSum += num(r.sum); }
    });
    t.sum = round(t.sum); t.waitingSum = round(t.waitingSum);
    return t;
  }

  return {
    snapshot: snapshot, addSnapshot: addSnapshot, priceHistory: priceHistory, priceJumps: priceJumps,
    seasons: seasons, groupSeasons: groupSeasons, together: together,
    shelfValue: shelfValue, returnDoc: returnDoc, returnTotals: returnTotals,
    MONTH_RU: MONTH_RU
  };
});
