...
> Везде `id` = UUID (первичный ключ), `org_id` = UUID (ссылка на организацию, RLS-фильтр).
> Суммы в копейках (bigint). `deleted_at` = мягкое удаление (NULL = запись жива).

---

**`users`** — профили пользователей (общая таблица, без org_id)
```
id · phone (unique) · name · created_at · last_seen_at
```

**`organizations`** — магазины / бизнесы
```
id · owner_id → users · name · settings(JSONB) · created_at
```
`settings` хранит: метод накоплений, логику ночной смены, целевую рентабельность, язык, часовой пояс.

**`org_members`** — кто работает в каком магазине и с какой ролью
```
id · org_id · user_id → users · role(owner|accountant|admin|staff)
   · permissions(JSONB) · status(active|suspended) · joined_at
```
Один пользователь — много строк (работает в нескольких магазинах).

---

**`accounts`** — счета магазина (касса, банк, резерв)
```
id · org_id · name · type(cash|bank|reserve) · icon · color
   · balance_kopecks · is_active · sort_order · created_at
```

**`categories`** — справочник статей (Закупка, ЗП, Аренда…)
```
id · org_id · name · type(income|expense|any) · icon · color
   · is_system · sort_order · created_at
```
`is_system = true` → нельзя удалить (системные категории).

---

**`transactions`** — все финансовые операции
```
id · org_id · client_uuid(unique) ← идемпотентность
   · type(income|expense|transfer|adjustment)
   · amount_kopecks
   · account_id → accounts
   · account_to_id → accounts (только для transfer)
   · category_id → categories
   · employee_id → employees
   · shift_id → shifts
   · comment · receipt_url
   · transaction_date · transaction_time
   · is_locked ← заблокировано Z-отчётом
   · deleted_at · created_at · created_by → users
   · custom_fields(JSONB)
```
`client_uuid` — генерируется на клиенте. Сервер отклоняет дубликат. Так работает защита от двойного нажатия.
...
```
id · org_id
   · shift_date · shift_number(1|2)
   · cashier_id → employees
   ...
   · withdrawals(JSONB)   ← [{name, account_id, amount_kopecks}, …]
   · status(open|closed|cancelled)
   · notes · created_at · created_by → users
```
...
**`suppliers`** — торговые представители и поставщики
```
id · org_id · name · phone · payment_schedule
   · credit_limit_kopecks · reliability_score(0–100)
   · status(active|inactive) · notes
   · custom_fields(JSONB) · created_at
```
`reliability_score` вычисляется из истории долгов (точность сумм, явка на выплаты).

**`debts`** — операции по долгам поставщика
```
id · org_id · supplier_id → suppliers
   · type(initial|purchase|payment|return|adjustment)
   · amount_kopecks  ← >0 долг растёт, <0 долг падает
   · account_id → accounts (откуда оплачено, nullable)
   · invoice_url · operation_date · status(active|cancelled)
   · comment · deleted_at · created_at · created_by → users
```
...
id · org_id · name · phone
   ...
   · salary_kopecks · hire_date · fire_date
   · status(active|fired) · user_id → users (если есть доступ в приложение)
   · custom_fields(JSONB) · created_at
```

**`timesheet_entries`** — табель рабочего времени
```
id · org_id · employee_id → employees · work_date
   · status(worked|half_day|day_off|sick|absent|vacation)
   · coefficient(1.00|0.50|0.00|…)  ← для нестандартных случаев
   · confirmed · confirmed_by → users · note · created_at
```
Приложение авто-заполняет по расписанию, бухгалтер проверяет и подтверждает.

---

**`events_log`** — лог всех действий (аудит + ИИ-обучение)
```
id · org_id · user_id → users
   · event_type  ← 'transaction.created', 'shift.closed', …
   · entity_type · entity_id
   · payload(JSONB) · created_at
```
Никогда не удаляется. Служит основой для ИИ-анализа паттернов.

**`custom_fields_meta`** — описание кастомных полей per-org
```
id · org_id · entity_type(supplier|employee|transaction|…)
   · field_key · field_name · field_type(text|number|date|select)
   · options(JSONB) ← для select · is_required · sort_order · created_at
```
