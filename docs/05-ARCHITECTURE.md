...
### Офлайн-режим: просмотр + очередь операций (путь A)
> Обновлено 2026-07-29 (утверждено владельцем, см. DECISIONS): офлайн больше не «только просмотр» — добавлена очередь новых операций (путь A). Полноценное офлайн-редактирование существующих записей (пути B/ElectricSQL/CRDT) — отдельное будущее решение.

Если интернет пропал:
- **Просмотр** — данные, загруженные до обрыва, доступны (Service Worker, stale-while-revalidate).
- **Новые операции** — вводятся и **копятся в очереди** (`offline_queue`, локально), досылаются при восстановлении связи. Идемпотентность по `client_uuid` — дубликаты сервер отсекает.
- **Изменение/удаление существующих записей** офлайн пока заблокировано (появится с путём B).

```
┌────────────────────────────────────────────┐
│  ⚠️  Нет связи — данные на 14:32           │
│  Новые операции сохранятся и уйдут          │
│  при восстановлении сети · N ждут отправки  │
└────────────────────────────────────────────┘
```

**Реализация:** Service Worker кэширует последние данные (stale-while-revalidate). Клиентская очередь — `app/js/offline-queue.js` (localStorage), встроена в слой `api.js` на вставке; автоотправка по событию `online` и при загрузке; статус-бар показывает число ждущих операций.
...
Конфликты сведены к минимуму: офлайн можно **создавать** новые операции (каждая со своим `client_uuid`), но не редактировать существующие — значит одновременная правка одной записи невозможна. Новые операции из очереди просто добавляются, дубликаты отсекаются по `client_uuid`. Если два устройства всё же изменили одно и то же (онлайн) — побеждает более поздняя по timestamp (last-write-wins). Для финансовых операций это приемлемо, потому что реестр неизменяемый (правки — сторно).
