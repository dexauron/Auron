...
# ── 2б. Проверки полного учёта ──────────────────────────────────────────
# Вторая половина продукта живёт в desktop/ и считает те же деньги. Её
# проверки — часть приложения, а не чужое хозяйство: мост между половинами
# ломается тихо, цифра просто становится другой.
# Тяжёлые прогоны (тысяча дней, разбор, экраны) здесь не гоняем — они идут
# минутами; перед пушем важны быстрые, ловящие перенос и связность.
...
if [ -d desktop/tests ]; then
  ...
  for t in "проверка-моста.js" "проверка-публикации.js" "проверка-настроек.js" "проверка-года.js"; do
    ...
    else bad "упал $t"; printf '%s\n' "$out" | tail -12 | sed 's/^/      /'; fi
  done
  [ "$FAIL" -eq 0 ] && good "все $m файлов зелёные"
else good "папки нет — пропускаем"; fi
