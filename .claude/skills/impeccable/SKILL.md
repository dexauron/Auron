---
name: impeccable
description: Дизайн рабочего интерфейса: композиция, иерархия, типографика, отступы, цвет, состояния, доступность, плотность экрана. Обязательно используй, когда владелец говорит «некрасиво», «неудобно», «непонятно», «сделай красиво», «переделай экран», «выглядит дёшево», когда добавляется или переделывается любой экран, карточка, таблица, форма, окно или пустое состояние, и перед показом владельцу нового вида. Подходит нашим рабочим экранам — Касса, Товары, Поставщики, Журнал, Аналитика, Настройки. Не для задач без интерфейса.
version: 4.0.4
user-invocable: true
...
> Взяты только ЗНАНИЯ. Скрипты автора (браузерный режим, генератор палитр,
> генерация картинок, детектор) в проект не копировались: наш фронт — один
> файл `webapp/Index.html` внутри Google Apps Script, их тулинг под него не
> работает. Всё, что скил советует, применяю руками.
> Смотреть на экран — через наш скил `look`, а не через браузер автора.
> Проверять перед пушем — через `preflight`.
...
1. **(Правка для этого проекта.)** Оригинальный шаг запускал `scripts/context.mjs`. Скрипты автора в проект не положены — см. врезку в начале файла. Вместо него: прочитай `docs/03-SCREENS.md` и `docs/04-DESIGN-SYSTEM.md` — это наши PRODUCT.md и DESIGN.md, — и посмотри на экран через скил `look`. Браузерных режимов, генерации картинок и детектора здесь нет.
