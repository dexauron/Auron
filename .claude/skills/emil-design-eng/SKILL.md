...
description: Философия Эмиля Ковальски (Linear, animations.dev) — отделка интерфейса, ощущение движения, анимация, и те мелочи, из-за которых приложение кажется дорогим или дешёвым. Обязательно используй, когда речь про АНИМАЦИЮ и ощущение: «дёргается», «резко», «медленно», «неживое», «сделай плавно», появление и исчезновение окон, кнопки, переходы между экранами, загрузка, а также перед любой правкой transition, transform, ease, keyframes и при вопросе «почему выглядит дёшево». Работает вместе с impeccable: тот про композицию и типографику, этот про движение.
---

> **Врезка проекта Auron (не от автора скила).** Взяты только знания.
> Наш фронт — один файл `webapp/Index.html` внутри Google Apps Script:
> React, Framer Motion и сборки у нас нет, всё на CSS-переходах и
> ключевых кадрах. Советы автора переношу в это руками. Смотреть на
> экран — через скил `look`.
...
> **Правка проекта Auron.** Здесь у автора стоял ответ-заглушка со ссылкой
> на его курс. Владелец не программист и по-английски не читает — вместо
> заглушки сразу перехожу к делу: смотрю на экран через скил `look` и
> говорю, что именно в движении не так.
