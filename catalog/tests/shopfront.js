...
(async () => {
  ...
  chk(/Молоко Простоквашино 3,2% 930 мл/.test(v.grid),
    `фасовка вынесена из названия отдельной строкой (${(v.grid.match(/Молоко[^0-9]*930 мл/) || [''])[0]})`);
  ...
  chk(/89 ₽ 101 ₽ −12 ₽/.test(v.grid), `видно, что подешевело и на сколько (${(v.grid.match(/89 ₽[^А-Я]{0,20}/) || [''])[0]})`);
