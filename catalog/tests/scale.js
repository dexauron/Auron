...
(async () => {
  ...
  // копейки обязательны: это то самое число, которое человек увидит на кассе
  chk(/181,50 ₽/.test(tag), `сумма к оплате посчитана с копейками (${(tag.match(/К оплате [^·]*/) || [''])[0].slice(0, 30)})`);
