...
(async () => {
  ...
  { /* описание товара из 1С уезжает покупателю на ценник.
       Владелец: «если добавишь ко всем номенклатурам описание — было бы очень
       хорошо». Сами описания придумывать нельзя, но если в отчёте есть такая
       колонка, товар должен её забрать. Повтор названия описанием не считаем:
       в 1С «Характеристику» часто заполняют тем же названием. */
    const r = await imp([
      ['Прайс-лист'], [],
      ['Номенклатура', 'Код товара', 'Описание', 'Розничная цена', 'Количество'],
      ['Молоко 3,2%', '501', 'Пастеризованное, срок 10 суток', '89', '4'],
      ['Кефир 1%', '502', 'Кефир 1%', '75', '2'],
    ]);
    const milk = (r.products || []).find((p) => /Молоко/.test(p.name)) || {};
    const kef = (r.products || []).find((p) => /Кефир/.test(p.name)) || {};
    chk(milk.description === 'Пастеризованное, срок 10 суток',
      `описание из 1С попало в товар (${milk.description || 'нет'})`);
    chk(!kef.description, 'описание, повторяющее название, не берём');
  }
  { /* «Неликвидные товары» — единственный отчёт, где есть настоящая дата
       последнего поступления. Раньше дату завоза угадывали по дате цены
       поставщика, а цена могла обновиться и без поступления. */
    const r = await page.evaluate((rr) => {
      ...
      return { type, products: s.products };
    }, [
      ['Неликвидные товары'], [],
      ['Параметры:', '', 'Период: 01.05.2026 - 01.08.2026'],
      [],
      ['Склад', '', '', '', '', 'Общий приход', 'Поступление', 'Дата последнего поступления', 'Конечный остаток', 'Продажи'],
      ['Номенклатура'],
