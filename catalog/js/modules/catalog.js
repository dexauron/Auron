 ...
 * известен: каждое взвешивание даёт новый код. Поэтому обычный поиск по
 * штрихкоду такую этикетку не находил, хотя товар в каталоге есть.
 *
 * Разобрано по настоящей этикетке магазина:
 *   2 305940 003301   «П/К Чеченская В/У», 0,330 кг, 550 ₽/кг, 181,50 ₽
 *   ──┬ ──┬── ──┬── ┬
 *     │    │     │   └ контрольная цифра EAN-13 (проверена — сходится)
 *     │    │     └──── 00330 = 330 граммов
 *     │    └────────── 05940 = код товара 5940 (ведущий ноль не значащий)
 *     └─────────────── 23 = «внутри магазина, весовой»
 *
 * Пять цифр веса — это именно ГРАММЫ, а не сумма: сумма на этой этикетке
 * 181,50 ₽, то есть 18150, а в коде стоит 00330. Проверять это надо каждый
 * раз при смене настроек весов — бывает и наоборот.
 *
 * Разбор здесь только считает. Проверяет догадку тот, кто ищет товар:
 * код должен найтись в каталоге, а вес — быть похожим на правду. Так мы не
 * примем за весовой обычный штрихкод, который тоже начинается с двойки. */
const SCALE_MIN_G = 10;        // меньше десяти граммов не взвешивают
const SCALE_MAX_G = 30000;     // тридцать килограммов — предел бытовых весов
/* Префикс весов этого магазина. Владелец подтвердил: весы одни и печатают
 * «23». Держим список, а не одно значение: в магазине с несколькими весами
 * у каждого отдела бывает свой префикс, и тогда сюда добавляется строка. */
const SCALE_PREFIX = ['23'];

export function parseScaleBarcode(text) {
  const s = String(text || '').replace(/\D/g, '');
  if (s.length !== 13 || !SCALE_PREFIX.includes(s.slice(0, 2))) return null;
  // контрольная цифра: если не сходится, это не штрихкод, а мусор от камеры
  let sum = 0;
  for (let i = 0; i < 12; i++) sum += Number(s[i]) * (i % 2 ? 3 : 1);
  if ((10 - (sum % 10)) % 10 !== Number(s[12])) return null;
  const code = String(Number(s.slice(2, 7)));      // ведущие нули отбрасываем
  const grams = Number(s.slice(7, 12));
  if (!(grams >= SCALE_MIN_G && grams <= SCALE_MAX_G)) return null;
  return { code, grams, kg: grams / 1000 };
}
